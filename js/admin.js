...
function renderEventList_() {
  ...
  list.innerHTML = allEvents_.map(ev => {
    ...
    // 「開催結果」ボタンはカードの外・右側に、カードと同じ高さで並べる
    // （2026-08-25 レイアウト変更。カードとボタンは兄弟要素なので <a> の入れ子にはならない）。
    return `
      <div class="event-card-row">
        ...
        <a class="ev-report-btn" href="report.html?event=${esc_(ev.eventId)}" target="_blank">📊<br>開催結果</a>
      </div>`;
  }).join('');
}
...
// ⚠ CSV出力の共通ヘルパー（downloadCsv_ / downloadCsvSjis_ / encodeSjis_ など）は
//   js/csv-util.js へ切り出した（2026-08-24）。企業ページからも使うため。
//   admin.html は csv-util.js を admin.js より前に読み込んでいる。
 ...
 * ⚠ 送り状システムの取込は列の位置で行われ、見出し名は照合されない（2026-09-17 実機確認）。
 *   そのため住所1・住所2・名称1の見出しには文字数上限の注釈を付けている。列の並びは変えないこと。
 * 対応関係（ユーザー確認済み・2026-08-05、住所2/名称1は 2026-09-17 変更）:
 *   - お届け先電話番号 ← 担当者電話（代表電話ではなく、実際にパスを受け取る担当者へ連絡がつくため）
 *   - お届け先住所1 ← 住所
 *   - 部署名あり: お届け先住所2 ← 企業名正式 / お届け先名称1 ← 部署名
 *     （旧仕様は住所2=部署名・名称1=企業名で、印字が「部署名→企業名」の順になっていた）
 *   - 部署名なし: お届け先住所2 ← 空欄 / お届け先名称1 ← 企業名正式
 *     （名称1が空欄だと送り状システムの取込がエラーになる。部署名は企業フォームで任意項目）
 *   - お届け先名称2 ← 担当者名+「様」
...
function downloadEntryShippingCsv_() {
  ...
  // 文字数上限（送り状システムの仕様）。超過しても出力は止めず、出力後に確認を促すだけにする
  //   （入力制限を掛けると不自由になるため、不都合が出た場合のみ手作業で直す運用。2026-09-17 ユーザー判断）。
  //   文字数は全角・半角とも1文字として数える。
  const LIMITS = { 'お届け先住所1': 26, 'お届け先住所2': 30, 'お届け先名称1': 30 };
  const cols = ['お届け先電話番号', '郵便番号', 'お届け先県名', 'お届け先住所1', 'お届け先住所2',
                'お届け先名称1', 'お届け先名称2', '出荷通知メール希望区分', '記事'];
  const header = cols.map(c => LIMITS[c] ? `${c}（${LIMITS[c]}文字以内）` : c).join(',');
  const overLimit = [];
  const rows = targets.map(e => {
    const contact = String(e['担当者名'] || '').trim();
    const dept    = String(e['部署名'] || '').trim();
    const company = e['企業名正式'] || '';
    const values = [
      e['担当者電話'] || '',
      e['郵便番号'] || '',
      e['都道府県'] || '',
  ...
  if (overLimit.length) {
    const shown = overLimit.slice(0, 10).join('\n');
    const more  = overLimit.length > 10 ? `\n…他 ${overLimit.length - 10}件` : '';
    window.alert(
      '文字数が上限を超えている項目があるため、印刷が途切れている可能性があります。\n' +
      '実際の印刷内容を確認してください。\n\n' + shown + more
    );
  }
  showToast_(issues.length
