# タスク履歴 — FG Event Platform（アーカイブ）

> **このファイルはアーカイブです。更新不要。**
>
> 旧 `CLAUDE_TASKS.md`（ルート直下）を 2026-07-29 にリネーム・移動したもの。
> **恒久的な作業ルール・設計上の確定事項は `CLAUDE.md` に移設済み**であり、
> そちらが正。このファイルは 2026-06-29 時点までの進捗と、
> 完了済みタスク（T2 / T3 / T7）の当時の指示書を、経緯の記録として残す。
>
> 記載内容は当時のスナップショットのため、**現在の実装と一致しない箇所がある**。
> 実装の現状は `docs/AUDIT_2026-07-29.md`、その後の変更は `git log` を参照。
>
> 旧記述にある `handoff.md`（ルート直下）は存在しない。
...
> 【2026-07-29 追記】この再デプロイは実施済み。以降も複数回の再デプロイを経ているため、
> **上記の警告は当時の記録であり、現在の未対応事項ではない。**
...
> 旧「作業の進め方（Code向け）」の節は `CLAUDE.md` §10 に移設した。
> `handoff.md` 参照や「コミット毎に CHANGELOG 追記」など現行と異なる記述を含むため、
> ここには残さない。
