...
- GAS: **再デプロイ必須**
- 申し送り/注意点: **`docs/gas-patches/api.gs.final.txt`はPR #70（企業申込の上書き型化）以降、
  再デプロイの実施確認を取れていない。** 本エントリを含め、PR #70・#72・本エントリの
  GAS変更をまとめて1回で反映すること。個別に確認しながら順次反映する場合は、
  それぞれの変更内容（本ファイル該当エントリ参照）を踏まえて動作確認すること。
