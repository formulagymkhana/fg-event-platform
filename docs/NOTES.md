...
## [未解決] 学生マスターシートが STUDENT_COLS と2列ズレている（性別が未保存）（2026-06-08 記録）
- 状況: LIVE の学生マスターは16列（studentId..登録種別）で、`性別`・`所属校出場` が欠落。一方 admin.gs の `STUDENT_COLS` は18列。前回 性別/所属校出場 を STUDENT_COLS に足した際、既存シートへの列追加指示が漏れていた。
- 影響: `registerWalkIn` はヘッダ名照合で書くため、列が無い `性別`（当日フォームで収集済み）が黙って捨てられている。
- 対処: シートのヘッダを STUDENT_COLS(18列) に「列の挿入」で揃える（生年月日↔メールアドレス間に `性別`、電話番号↔SNS同意間に `所属校出場`）。順序厳守（importStudents は位置依存でヘッダ上書きするため）。
- 申し送り: 今後 STUDENT_COLS を変更したら、必ず LIVE シートの列も同時に更新する（手動シートとコードが乖離しやすい）。

## [解決] 当日フォーム「規則誓約同意」(#15) は不要だった（2026-06-08 記録）
- 結論: 撤回。見学・応援学生は競技に出場しないため大会規則書・誓約書の同意は対象外（実フォームでも応援学生区分には無かった）。フロント/GAS patch から削除済み。仕様書 line139 の記載はドライバー区分からのコピペ要約ミスと判断（エラー文「出場できません」が見学・応援用の他項目と不整合）。
