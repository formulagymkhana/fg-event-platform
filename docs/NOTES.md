...
## [未解決] 未登録大学の確定フロー(`actionAdminConfirmUniversity_`)に大学統合と同種の脆弱性（2026-08-04 記録）
- 状況: 大学統合機能（`actionAdminMergeUniversity_`）の4回にわたる外部レビューで、
  ...
  **今回のハードニングを一切受けておらず、同種の脆弱性を抱えたまま**。
  具体的には: LockServiceを使っていないため、2つの大学を同時に確定操作すると
  同じ確定コードを採番しうる。書き換え途中で失敗すると大学マスターだけ確定済みに
  なりUIから再実行できない。QR閲覧ログ・womenPairingsが更新対象外。
  非アクティブイベントにも書き込む。セル単位更新でタイムアウトしやすい。
- 懸念/論点: 実害が報告されているわけではないが、大学統合と全く同じ設計の穴が
  未対応のまま残っている。優先度は高くないが、いずれ同じレビュー観点で
  再確認する価値がある。
- 案: 大学統合で確立した「大学統合履歴」型の再開可能エンジンへ、確定フローも
  寄せる（reconciled/reviewされた設計を流用できる）。
